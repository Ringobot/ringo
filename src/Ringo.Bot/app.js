...
const _health = require("./services/healthService");
...
intents.matches('Like Artist', (session, args, next) => __awaiter(this, void 0, void 0, function* () {
    ...
    if (spotifyUri) {
        ...
        try {
            ...
            let msg2 = yield _messages.recommendArtist(session, artist);
            if (msg2)
        ...
        try {
            ...
            if (match[0]) {
                ...
                let msg2 = yield _messages.recommendArtist(session, match[1]);
                if (msg2)
                    session.endDialog(msg2);
                else
                    session.endDialog();
